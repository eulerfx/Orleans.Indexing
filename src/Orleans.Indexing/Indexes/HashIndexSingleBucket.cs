...
namespace Orleans.Indexing
{
    ...
    public abstract class HashIndexSingleBucket<K, V> : Grain, ITransactionalLookupIndex<K, V>, IHashIndexSingleBucketInterface<K, V> where V : class, IIndexableGrain
    {
        ...
        ITransactionalState<HashIndexBucketState<K, V>> maybeTransactionalState;
        string nonTransactionalStorageProviderName;

        bool IsTransactional => this.nonTransactionalStorageProviderName == null;
        ...
        void EnsureStateInitialized(HashIndexBucketState<K, V> state)
        {
        ...
        async Task<TResult> ReadStateAsync<TResult>(Func<HashIndexBucketState<K, V>, TResult> readFunc)
        {
        ...
        async Task<TResult> WriteStateAsync<TResult>(Func<HashIndexBucketState<K, V>, TResult> updateFunc)
        {
        ...
        Task WriteStateAsync(Action<HashIndexBucketState<K, V>> updateFunc)
            => this.WriteStateAsync(state => { updateFunc(state); return true; });

        Task WriteStateAsync() => this.WriteStateAsync(_ => { });
        ...
        /// </summary>
        HashSet<int> pendingWriteRequests;
        ...
        public async Task<bool> DirectApplyIndexUpdateBatch(Immutable<IDictionary<IIndexableGrain, IList<IMemberUpdate>>> iUpdates, bool isUnique, IndexMetaData idxMetaData, SiloAddress siloAddress = null)
        {
            ...
            this.Logger.Trace(IndexingErrorCode.Indexing, $"Started calling DirectApplyIndexUpdateBatch with the following parameters: isUnique = {isUnique}, siloAddress = {siloAddress}, iUpdates = {MemberUpdate.UpdatesToString(iUpdates.Value)}");
            ...
            this.Logger.Trace(IndexingErrorCode.Indexing, $"Finished calling DirectApplyIndexUpdateBatch with the following parameters: isUnique = {isUnique}, siloAddress = {siloAddress}, iUpdates = {MemberUpdate.UpdatesToString(iUpdates.Value)}");
            return true;
        }
        ...
        async Task DirectApplyIndexUpdateNonPersistent(IIndexableGrain g, IMemberUpdate updt, bool isUniqueIndex, IndexMetaData idxMetaData, SiloAddress siloAddress)
        {
            // The target grain that is updated
            var updatedGrain = g.AsReference<V>(this.SiloIndexManager);
        ...
        async Task ApplyIndexUpdateTransactional(IIndexableGrain g, IMemberUpdate updt, bool isUniqueIndex, IndexMetaData idxMetaData, SiloAddress siloAddress)
        {
            // The target grain that is updated
            var updatedGrain = g.AsReference<V>(this.SiloIndexManager);
            ...
            var nextBucketIndexInterface = await this.WriteStateAsync(state =>
            {
                // UpdateBucketState is synchronous; note that no other thread can run concurrently before we reach an await operation,
                // when execution is yielded back to the Orleans scheduler, so no concurrency control mechanism (e.g., locking) is required.
                // 'fixIndexUnavailableOnDelete' indicates whether the index was still unavailable when we received a delete operation.
        ...
        async Task PersistIndexNonTransactional()
        {
        ...
        {
            this.Logger.Trace(IndexingErrorCode.Indexing, $"Streamed index lookup called for key = {key}");

            IIndexInterface<K, V> nextBucket = null;
            var entry = await this.ReadStateAsync(state =>
            {
        ...
        public async Task<V> LookupUniqueAsync(K key)
        {
            ...
            IIndexInterface<K, V> nextBucket = null;
            var result = await this.ReadStateAsync(state =>
            {
                if (state.IndexStatus != IndexStatus.Available)
                {
        ...
        public async Task<IOrleansQueryResult<V>> LookupAsync(K key)
        {
            ...
            var result = await ReadStateAsync(state =>
            {
                if (state.IndexStatus != IndexStatus.Available)
                {
