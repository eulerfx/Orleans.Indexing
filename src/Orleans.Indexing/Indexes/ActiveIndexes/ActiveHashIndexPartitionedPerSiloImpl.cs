...
//             => throw new NotSupportedException();
//
//         static GrainReference GetGrainReference(SiloIndexManager siloIndexManager, string indexName, SiloAddress siloAddress = null)
...
//
//             //get all silos
//             var hosts = await this.SiloIndexManager.GetSiloHosts(true);
//             var queriesToSilos = await Task.WhenAll(GetResultQueries(hosts, key));
...
//
//         ISet<Task<IOrleansQueryResult<IIndexableGrain>>> GetResultQueries(Dictionary<SiloAddress, SiloStatus> hosts, object key)
//         {
//             var queriesToSilos = new HashSet<Task<IOrleansQueryResult<IIndexableGrain>>>();
//
//             var i = 0;
//             var indexName = IndexUtils.GetIndexNameFromIndexGrain(this);
//
...
//             {
//                 //query each silo
//                 var grainReference = GetGrainReference(this.SiloIndexManager, indexName, siloAddress);
//                 var grainService = this.SiloIndexManager.GetGrainService<IActiveHashIndexPartitionedPerSiloBucket>(grainReference);
//                 queriesToSilos.Add(grainService.LookupAsync(/*result, */key));
//                 //TODO: a bug in OrleansStream currently prevents a GrainService from working with streams.
//                 ++i;
//             }
//
...
//                 var firstFinishedTask = await Task.WhenAny(queriesToSilos);
//
...
//                 var partialResult = await firstFinishedTask;
